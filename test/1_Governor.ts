...
describe("Poolz Governor", () => {
    ...
    const createVaultSig = "setValue(uint256)"
    ...
    it("make sure poolzGovernor is the owner of vaultManager", async () => {
        expect(await governee.owner()).to.equal(poolzGovernor.address)
    })
    ...
    it("should calculate the correct role from contract address and selector", async () => {
        const role = await poolzGovernor.getRoleOfSelector(governee.address, createVaultSelector)
        const roleTs = getRoleOfSelector(governee.address, createVaultSelector)
    ...
    it("should add new function", async () => {
        const tx = await poolzGovernor.AddNewFunction(governee.address, createVaultSig, requiredVotes)
        const role = getRoleOfSelector(governee.address, createVaultSelector)
        const votes = await poolzGovernor.SelectorToRequiredVotes(governee.address, createVaultSelector)
        await expect(tx).to.emit(poolzGovernor, "RoleAdminChanged").withArgs(role, DEFAULT_ROLE, ADMIN_ROLE)
        await expect(tx).to.emit(poolzGovernor, "FunctionAdded").withArgs(governee.address, createVaultSelector, requiredVotes)
        expect(votes).to.equal(requiredVotes)
    })

    it("should remove function", async () => {
        const tx = await poolzGovernor.RemoveFunction(governee.address, createVaultSig)
        await expect(tx).to.emit(poolzGovernor, "FunctionRemoved").withArgs(governee.address, createVaultSelector)
        expect(await poolzGovernor.SelectorToRequiredVotes(governee.address, createVaultSelector)).to.equal(0)
        expect(1).to.equal(1)
    })
    ...
    describe("User CRUD", () => {
        it("should grant role of function to user", async () => {
            ...
            for( const [index, admin] of admins.entries()){
                const tx = await poolzGovernor.connect(admin).grantRoleOfFunction(governee.address, createVaultSig, user.address)
                const votes = await poolzGovernor.UsersToVotes(user.address, governee.address, createVaultSelector)
                const voteOfAdmin = await poolzGovernor.getUserVoteOf(user.address, governee.address, createVaultSelector, admin.address)
                if(index < admins.length - 1){
                    expect(votes).to.equal(index + 1)
                    expect(voteOfAdmin).to.equal(true)
                } else {
                    const role = getRoleOfSelector(governee.address, createVaultSelector)
                    await expect(tx).to.emit(poolzGovernor, "FunctionGranted").withArgs(governee.address, createVaultSelector, user.address)
                    await expect(tx).to.emit(poolzGovernor, "RoleGranted").withArgs(role, user.address, admin.address)
                }
            }
            for (const admin of admins) {
                expect(await poolzGovernor.getUserVoteOf(user.address, governee.address, createVaultSelector, admin.address)).to.be.false
            }
            const votes = await poolzGovernor.UsersToVotes(user.address, governee.address, createVaultSelector)
            expect(votes).to.equal(0)
            const role = getRoleOfSelector(governee.address, createVaultSelector)
        ...
        it("should revoke role of function from user", async () => {
            ...
            if(!user) throw new Error("user is undefined")
            const tx = await poolzGovernor.revokeRoleOfFunction(governee.address, createVaultSig, user.address)
            const role = getRoleOfSelector(governee.address, createVaultSelector)
            expect(await poolzGovernor.hasRole(role, user.address)).to.be.false
            await expect(tx).to.emit(poolzGovernor, "FunctionRevoked").withArgs(governee.address, createVaultSelector, user.address)
